...
def _arch(version):
    ...
    uname = os.uname()
    if uname[0] == 'Linux' and uname[4].startswith(
            'i') and uname[4].endswith('86'):
        return 'LINUX'
...
def _init_environ(path=None, version=None):
    ...
    lib_dir = '%s/%s/LIBR/INTELCMP' % (
        os.environ['AIPS_VERSION'], os.environ['ARCH'])
...
def _set_version(version):
    ...
    os.environ['LOAD'] = os.environ['AIPS_VERSION'] + \
        '/' + os.environ['ARCH'] + '/LOAD'
...
def _make_da00(da00_path=None, force=False):
    ...
    if not os.path.exists(da00_path) or force:
        template_path = os.environ['AIPS_VERSION'] + \
            '/' + os.environ['ARCH'] + '/TEMPLATE'
...
def _filaip(force=False):
    ...
    template_dir = os.environ['AIPS_VERSION'] + \
        '/' + os.environ['ARCH'] + '/TEMPLATE'
    ...
    if run_filaip:
        os.system('echo 8 2 | %s/%s/LOAD/FILAIP.EXE' %
                  (os.environ['AIPS_VERSION'], os.environ['ARCH']))
        pass
    if da00:
